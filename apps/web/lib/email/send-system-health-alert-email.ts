...
export async function sendSystemHealthAlertEmail(input: {
  ...
  const subject = `🟡 [RISCO ALTO] ACC - FALHA OPERACIONAL - ${input.projectName}`;
  ...
  const html = `<table role="presentation" width="100%" style="background:#fff;color:#000;padding:24px"><tr><td><p style="color:#000;font-weight:700">ACC · SUPERVISÃO OPERACIONAL</p><p><span style="display:inline-block;background:#FFD600;color:#000;font-weight:700;padding:6px 12px;border-radius:999px">RISCO ALTO</span></p><p style="color:#000"><strong>Fonte:</strong> ${escapeHtml(input.sourceType)}</p><p style="color:#000"><strong>Problema:</strong> ${escapeHtml(input.summary)}</p><p><a href="${escapeHtml(input.auditUrl)}" title="Causa do alerta: ${escapeHtml(input.summary)}" style="display:inline-block;background:#111827;color:#fff;text-decoration:none;padding:10px 18px;border-radius:6px;font-weight:700">ABRIR AUDITORIA</a></p><p style="color:#6b7280;font-size:11px">O ACC suprimiu repetições desta mesma falha para evitar excesso de e-mails.</p></td></tr></table>`;

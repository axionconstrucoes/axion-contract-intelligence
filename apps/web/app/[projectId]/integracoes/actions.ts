...
import type { ValidateConstrumanagerConnectionState } from "./actions-state";
import type {
...
function sanitizeIntegrationConnectionError(error: unknown): string {
...
function classifyConstrumanagerConnectionFailure(
  message: string
): "ATENCAO" | "ERRO" {
  if (
